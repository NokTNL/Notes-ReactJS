...
// Therefore the simplest way of initialising is to tell the type in advance, and then initialise with an default empty value
// - You can use `as`, called TYPE ASSERTION, which tells TS you are sure that it WILL have that type later on (we are sure about that as we always initialise the context value in the provider with that type)
// !!! make sure you DID INITIALISE the context's provider properly, otherwise will throw RUNTIME error when using it
...
// - Alternatively, tell TS that the context value can be of null type as well, using UNION type
// Doing this and TS will warn you that the context could be null
const TodosContext2 = createContext<TodosContextType | null>(null);
...
const TodosContextProvider = (
  props: PropsWithChildren<{ extraProp?: string }>
) => {
